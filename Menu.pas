...
uses
  ...
  FireDAC.Phys.MySQLDef, FireDAC.VCLUI.Wait, frxExportPDF;
...
type
  TFRMMENU = class(TIWAppForm)
    ...
    procedure RELPRODUTOSAsyncClick(Sender: TObject; EventParams: TStringList);
  public
  end;
...
procedure TFRMMENU.RELPRODUTOSAsyncClick(Sender: TObject;
...
begin
   ...
   oExportFilter.FileName := GetCurrentDir + '\wwwroot\rel\produtos.pdf';
   rel_produtos.PrepareReport(True);
   rel_produtos.Export(oExportFilter);
   WebApplication.CallBackResponse.AddJavaScriptToExecute('window.open("rel/produtos.pdf");');
